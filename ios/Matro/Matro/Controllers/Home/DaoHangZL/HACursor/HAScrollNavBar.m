...
- (void)buttonClick:(UIButton *)button{
    ...
    NSLog(@"off ---> %f,button.tag值为：%ld",offX,button.tag);
    ...
    /*zhoulu*/
    //创建一个消息对象
    NSNotification * notice = [NSNotification notificationWithName:HOMEVIEW_BUTTON_INDEX_NOTIFICATION object:nil userInfo:@{@"tag":tagStr}];
    //发送消息
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    //创建一个消息对象
    NSNotification * notice = [NSNotification notificationWithName:HOMEVIEW_BUTTON_INDEX_NOTIFICATION object:nil userInfo:@{@"tag":numStr}];
    //发送消息
    [[NSNotificationCenter defaultCenter]postNotification:notice];
